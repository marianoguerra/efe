...
pp({'fun', Line, {function, FName, Arity}}, Ctx) ->
    beside(text("&"), wrap(text("/"), pp_call_pos({atom, Line, FName}, "", Ctx), pp({integer, Line, Arity}, Ctx)));
pp({'fun', _, {function, MName, FName, Arity}}, Ctx) ->
    beside(text("&"), wrap(text("/"), wrap(dot_f(), pp_call_pos(MName, ":", Ctx), pp_call_pos(FName, "", Ctx)), pp(Arity, Ctx)));
...
pp({named_fun, _, AName, Clauses}, Ctx) ->
    above(sep([text("fn " ++ transform_var_name(AName)), pp_case_clauses(Clauses, Ctx)]),
          text("end"));
pp({function, _, Name, Arity, Clauses}, Ctx) ->
    IsExported = function_exported(Ctx, Name, Arity),
    DefKw = if IsExported -> "def"; true -> "defp" end,
...
pp({'case', _, Expr, Clauses}, Ctx) ->
    above(parc(Ctx, [text("case"), sep([pp(Expr, Ctx), dok()])]),
          above(nestc(Ctx, pp_case_clauses(Clauses, Ctx)),
                text("end")));
...
pp({'receive', _, [], AfterExpr, AfterBody}, Ctx0) ->
    Ctx = reset_prec(Ctx0),
    abovel([besidel([text("receive do"), text(" after "), sep([pp(AfterExpr, Ctx), sarrow_f()])]),
...
otuple_f() -> floating(text("{")).
ctuple_f() -> floating(text("}")).
...
pp_cons(V, Ctx) ->
    case cons_to_list(V, []) of
        ...
        [A | B] when not is_list(B) ->
            wrap_list(followc(Ctx, beside(wrap_list(pp_items(A, Ctx)), text(" |")), pp(B, Ctx)));
...
pp_if_clauses([Clause | Clauses=[_|_]], Ctx) ->
    above(pp_if_clause(Clause, Ctx),
          pp_if_clauses(Clauses, Ctx)).

pp_if_clause({clause, _, _, Guards, Body}, Ctx) ->
    pp_header_and_body_no_end(Ctx,
                       pp_if_header(Ctx, "", Guards),
                       Body).
...
pp_case_clause({clause, _, Patterns, Guards, Body}, Ctx) ->
    pp_header_and_body_no_end(Ctx,
                       followc(Ctx,
                               pp_args_inn(Patterns, Ctx),
                               beside(text("when "), sep([pp_guards(Guards, Ctx), sarrow_f()]))),
                       Body).
...
pp_try_catch_case({clause, _, [{tuple, _, TItems}], [], Body}, Ctx) ->
    pp_header_and_body_no_end(Ctx,
                       sep([pp_try_catch_case_items(TItems, Ctx), sarrow_f()]),
                       Body);
...
is_autoimported(abs, 1) -> true;
is_autoimported(apply, 2) -> true;
is_autoimported(apply, 3) -> true;
is_autoimported(atom_to_binary, 2) -> true;
is_autoimported(atom_to_list, 1) -> true;
is_autoimported(binary_part, 2) -> true;
is_autoimported(binary_part, 3) -> true;
is_autoimported(binary_to_atom, 2) -> true;
is_autoimported(binary_to_existing_atom, 2) -> true;
is_autoimported(binary_to_float, 1) -> true;
is_autoimported(binary_to_integer, 1) -> true;
is_autoimported(binary_to_integer, 2) -> true;
is_autoimported(binary_to_list, 1) -> true;
is_autoimported(binary_to_list, 3) -> true;
is_autoimported(binary_to_term, 1) -> true;
is_autoimported(binary_to_term, 2) -> true;
is_autoimported(bit_size, 1) -> true;
is_autoimported(bitstring_to_list, 1) -> true;
is_autoimported(byte_size, 1) -> true;
is_autoimported(ceil, 1) -> true;
is_autoimported(check_old_code, 1) -> true;
is_autoimported(check_process_code, 2) -> true;
is_autoimported(check_process_code, 3) -> true;
is_autoimported(date, 0) -> true;
is_autoimported(delete_module, 1) -> true;
is_autoimported(demonitor, 1) -> true;
is_autoimported(demonitor, 2) -> true;
is_autoimported(disconnect_node, 1) -> true;
is_autoimported(element, 2) -> true;
is_autoimported(erase, 0) -> true;
is_autoimported(erase, 1) -> true;
is_autoimported(error, 1) -> true;
is_autoimported(error, 2) -> true;
is_autoimported(exit, 1) -> true;
is_autoimported(exit, 2) -> true;
is_autoimported(float, 1) -> true;
is_autoimported(float_to_binary, 1) -> true;
is_autoimported(float_to_binary, 2) -> true;
is_autoimported(float_to_list, 1) -> true;
is_autoimported(float_to_list, 2) -> true;
is_autoimported(floor, 1) -> true;
is_autoimported(garbage_collect, 0) -> true;
is_autoimported(garbage_collect, 1) -> true;
is_autoimported(garbage_collect, 2) -> true;
is_autoimported(get, 0) -> true;
is_autoimported(get, 1) -> true;
is_autoimported(get_keys, 0) -> true;
is_autoimported(get_keys, 1) -> true;
is_autoimported(group_leader, 0) -> true;
is_autoimported(group_leader, 2) -> true;
is_autoimported(halt, 0) -> true;
is_autoimported(halt, 1) -> true;
is_autoimported(halt, 2) -> true;
is_autoimported(hd, 1) -> true;
is_autoimported(integer_to_binary, 1) -> true;
is_autoimported(integer_to_binary, 2) -> true;
is_autoimported(integer_to_list, 1) -> true;
is_autoimported(integer_to_list, 2) -> true;
is_autoimported(iolist_size, 1) -> true;
is_autoimported(iolist_to_binary, 1) -> true;
is_autoimported(is_alive, 0) -> true;
is_autoimported(is_atom, 1) -> true;
is_autoimported(is_binary, 1) -> true;
is_autoimported(is_bitstring, 1) -> true;
is_autoimported(is_boolean, 1) -> true;
is_autoimported(is_float, 1) -> true;
is_autoimported(is_function, 1) -> true;
is_autoimported(is_function, 2) -> true;
is_autoimported(is_integer, 1) -> true;
is_autoimported(is_list, 1) -> true;
is_autoimported(is_map, 1) -> true;
is_autoimported(is_map_key, 2) -> true;
is_autoimported(is_number, 1) -> true;
is_autoimported(is_pid, 1) -> true;
is_autoimported(is_port, 1) -> true;
is_autoimported(is_process_alive, 1) -> true;
is_autoimported(is_record, 2) -> true;
is_autoimported(is_record, 3) -> true;
is_autoimported(is_reference, 1) -> true;
is_autoimported(is_tuple, 1) -> true;
is_autoimported(length, 1) -> true;
is_autoimported(link, 1) -> true;
is_autoimported(list_to_atom, 1) -> true;
is_autoimported(list_to_binary, 1) -> true;
is_autoimported(list_to_bitstring, 1) -> true;
is_autoimported(list_to_existing_atom, 1) -> true;
is_autoimported(list_to_float, 1) -> true;
is_autoimported(list_to_integer, 1) -> true;
is_autoimported(list_to_integer, 2) -> true;
is_autoimported(list_to_pid, 1) -> true;
is_autoimported(list_to_port, 1) -> true;
is_autoimported(list_to_ref, 1) -> true;
is_autoimported(list_to_tuple, 1) -> true;
is_autoimported(load_module, 2) -> true;
is_autoimported(make_ref, 0) -> true;
is_autoimported(map_get, 2) -> true;
is_autoimported(map_size, 1) -> true;
is_autoimported(max, 2) -> true;
is_autoimported(min, 2) -> true;
is_autoimported(module_loaded, 1) -> true;
is_autoimported(monitor, 2) -> true;
is_autoimported(node, 0) -> true;
is_autoimported(node, 1) -> true;
is_autoimported(nodes, 0) -> true;
is_autoimported(nodes, 1) -> true;
is_autoimported(now, 0) -> true;
is_autoimported(open_port, 2) -> true;
is_autoimported(pid_to_list, 1) -> true;
is_autoimported(port_close, 1) -> true;
is_autoimported(port_command, 2) -> true;
is_autoimported(port_command, 3) -> true;
is_autoimported(port_connect, 2) -> true;
is_autoimported(port_control, 3) -> true;
is_autoimported(port_to_list, 1) -> true;
is_autoimported(pre_loaded, 0) -> true;
is_autoimported(process_flag, 2) -> true;
is_autoimported(process_flag, 3) -> true;
is_autoimported(process_info, 1) -> true;
is_autoimported(process_info, 2) -> true;
is_autoimported(processes, 0) -> true;
is_autoimported(purge_module, 1) -> true;
is_autoimported(put, 2) -> true;
is_autoimported(ref_to_list, 1) -> true;
is_autoimported(register, 2) -> true;
is_autoimported(registered, 0) -> true;
is_autoimported(round, 1) -> true;
is_autoimported(self, 0) -> true;
is_autoimported(setelement, 3) -> true;
is_autoimported(size, 1) -> true;
is_autoimported(spawn, 1) -> true;
is_autoimported(spawn, 2) -> true;
is_autoimported(spawn, 3) -> true;
is_autoimported(spawn, 4) -> true;
is_autoimported(spawn_link, 1) -> true;
is_autoimported(spawn_link, 2) -> true;
is_autoimported(spawn_link, 3) -> true;
is_autoimported(spawn_link, 4) -> true;
is_autoimported(spawn_monitor, 1) -> true;
is_autoimported(spawn_monitor, 3) -> true;
is_autoimported(spawn_opt, 2) -> true;
is_autoimported(spawn_opt, 3) -> true;
is_autoimported(spawn_opt, 4) -> true;
is_autoimported(spawn_opt, 5) -> true;
is_autoimported(split_binary, 2) -> true;
is_autoimported(term_to_binary, 1) -> true;
is_autoimported(term_to_binary, 2) -> true;
is_autoimported(throw, 1) -> true;
is_autoimported(time, 0) -> true;
is_autoimported(tl, 1) -> true;
is_autoimported(trunc, 1) -> true;
is_autoimported(tuple_size, 1) -> true;
is_autoimported(tuple_to_list, 1) -> true;
is_autoimported(unlink, 1) -> true;
is_autoimported(unregister, 1) -> true;
is_autoimported(whereis, 1) -> true;
is_autoimported(_, _) -> false.

is_ex_autoimport(abs, 1) -> true;
is_ex_autoimport(apply, 2) -> true;
is_ex_autoimport(apply, 3) -> true;
is_ex_autoimport(binary_part, 3) -> true;
is_ex_autoimport(bit_size, 1) -> true;
is_ex_autoimport(byte_size, 1) -> true;
is_ex_autoimport(ceil, 1) -> true;
is_ex_autoimport(exit, 1) -> true;
is_ex_autoimport(floor, 1) -> true;
is_ex_autoimport(hd, 1) -> true;
is_ex_autoimport(is_atom, 1) -> true;
is_ex_autoimport(is_binary, 1) -> true;
is_ex_autoimport(is_bitstring, 1) -> true;
is_ex_autoimport(is_boolean, 1) -> true;
is_ex_autoimport(is_float, 1) -> true;
is_ex_autoimport(is_function, 1) -> true;
is_ex_autoimport(is_function, 2) -> true;
is_ex_autoimport(is_integer, 1) -> true;
is_ex_autoimport(is_list, 1) -> true;
is_ex_autoimport(is_map, 1) -> true;
is_ex_autoimport(is_number, 1) -> true;
is_ex_autoimport(is_pid, 1) -> true;
is_ex_autoimport(is_port, 1) -> true;
is_ex_autoimport(is_reference, 1) -> true;
is_ex_autoimport(is_tuple, 1) -> true;
is_ex_autoimport(length, 1) -> true;
is_ex_autoimport(make_ref, 0) -> true;
is_ex_autoimport(map_size, 1) -> true;
is_ex_autoimport(max, 2) -> true;
is_ex_autoimport(min, 2) -> true;
is_ex_autoimport(node, 0) -> true;
is_ex_autoimport(node, 1) -> true;
is_ex_autoimport(round, 1) -> true;
is_ex_autoimport(self, 0) -> true;
is_ex_autoimport(spawn, 1) -> true;
is_ex_autoimport(spawn, 3) -> true;
is_ex_autoimport(spawn_link, 1) -> true;
is_ex_autoimport(spawn_link, 3) -> true;
is_ex_autoimport(spawn_monitor, 1) -> true;
is_ex_autoimport(spawn_monitor, 3) -> true;
is_ex_autoimport(throw, 1) -> true;
is_ex_autoimport(tl, 1) -> true;
is_ex_autoimport(trunc, 1) -> true;
is_ex_autoimport(tuple_size, 1) -> true;
is_ex_autoimport(_, _) -> false.
...
is_ex_reserved('when') -> true;
is_ex_reserved('and') -> true;
is_ex_reserved('or') -> true;
is_ex_reserved('not') -> true;
is_ex_reserved('in') -> true;
is_ex_reserved('fn') -> true;
is_ex_reserved('do') -> true;
is_ex_reserved('end') -> true;
is_ex_reserved('catch') -> true;
is_ex_reserved('rescue') -> true;
is_ex_reserved('after') -> true;
is_ex_reserved('else') -> true;
is_ex_reserved(_) -> false.
